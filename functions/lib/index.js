...
exports.setStorageCors = exports.captureSnapshot = exports.getTranscriptPublicAsia = exports.getTranscriptPublic = exports.getTranscript = exports.debugSnapshot = void 0;
...
exports.debugSnapshot = (0, https_1.onRequest)(async (req, res) => {
    const videoId = req.query.videoId || 'KAasBVVVU6M';
    const logs = [];
    ...
    try {
        ...
        if (ctRes.ok) {
            ...
            if (specRegex.test(html))
                log(`[Debug] CodeTabs: SUCCESS (Found spec)`);
            else
                log(`[Debug] CodeTabs: FAILURE (No spec)`);
        }
        else {
        ...
        log(`[Debug] CodeTabs Exception: ${e}`);
    }
    // 3. Test AllOrigins
    try {
        ...
        if (aoRes.ok) {
            ...
            const specRegex = /"spec":"(https?:[^"]+\|[^"]+)"/;
            if (specRegex.test(html))
                log(`[Debug] AllOrigins: SUCCESS (Found spec)`);
            else
                log(`[Debug] AllOrigins: FAILURE (No spec)`);
        }
    }
    catch (e) {
        log(`[Debug] AllOrigins Exception: ${e}`);
    }
    res.json({ logs });
});
...
async function fetchStoryboardSpecFromCodeTabs(videoId) {
    try {
        ...
        if (e instanceof Error)
            console.warn(`[CodeTabs] Stack: ${e.stack}`);
    }
    return null;
}
...
async function fetchImageMaybeViaProxy(url, videoId) {
    // 1. Try Direct
    try {
...
// One-time utility to set CORS for storage
exports.setStorageCors = (0, https_1.onRequest)(async (req, res) => {
    try {
