...
// @ts-ignore
export const getTranscript = onCall({ secrets: [TRANSCRIPT_PROXY_URL] }, async (request) => {
    ...
    try {
        ...
        throw new HttpsError("internal", `Failed to fetch transcript: ${error.message}`);
...
export const getTranscriptPublic = onRequest(
...
export const getTranscriptPublicAsia = onRequest(
...
async function fetchVideoUrlFromInnertube(videoId: string): Promise<{ url: string, userAgent: string, referer: string } | null> {
    const referer = `https://www.youtube.com/watch?v=${videoId}`;
    ...
    const attempts = [
        ...
        },
        {
            ...
            headers: {
                "content-type": "application/json",
                "accept": "application/json",
                "x-youtube-client-name": "56", // WEB_EMBED
                "x-youtube-client-version": "1.20240228.01.00",
            },
            body: {
                context: {
        ...
        },
        {
            ...
            headers: {
                "content-type": "application/json",
                "accept": "application/json",
                "x-youtube-client-name": "55", // TV_EMBEDDED
                "x-youtube-client-version": "4.20220223.00.00",
            },
            body: {
                context: {
        ...
        for (const attempt of attempts) {
            try {
                ...
                if (json.playabilityStatus?.status !== "OK") {
                    console.warn(`[innertube-video] ${attempt.name} playability: ${json.playabilityStatus?.status}`);
                }
                ...
                const usable = formats.filter((f: any) =>
                    f.url &&
...
async function fetchStoryboardSpecManual(videoId: string): Promise<any | null> {
    const referer = `https://www.youtube.com/watch?v=${videoId}`;
    ...
    for (const key of INNERTUBE_API_KEYS) {
        for (const attempt of attempts) {
            try {
                ...
                if (json.playabilityStatus?.status !== 'OK') {
                    console.log(`[ManualStoryboard] ${attempt.name} status: ${json.playabilityStatus?.status}`);
                }

                console.log(`[ManualStoryboard] ${attempt.name} keys: ${Object.keys(json).join(',')}`);
                if (json.storyboards) console.log(`[ManualStoryboard] storyboards keys: ${Object.keys(json.storyboards).join(',')}`);
                if (json.playerConfig) console.log(`[ManualStoryboard] playerConfig keys: ${Object.keys(json.playerConfig).join(',')}`);
                ...
                const spec = json.storyboards?.playerStoryboardSpecRenderer?.spec ||
                    json.playerConfig?.storyboardConfig?.playerStoryboardSpecRenderer?.spec;
...
function parseStoryboardSpec(spec: string): any {
...
async function fetchStoryboardSpecFromWatchHtml(videoId: string, userAgent: string): Promise<any | null> {
    try {
        const referer = `https://www.youtube.com/watch?v=${videoId}`;
...
async function fetchStoryboardSpecFromCodeTabs(videoId: string): Promise<any | null> {
    try {
...
async function captureSnapshotFromStoryboard(videoId: string, timestamp: number, ffmpegPath: string, totalDuration?: number): Promise<any | null> {


    console.log(`[Storyboard] Attempting fallback for ${videoId} at ${timestamp}s (total=${totalDuration}s)`);

    let bestBoard: any = null;
    ...
    try {
        ...
        const storyboards = (info.basic_info as any).storyboards;

        if (storyboards && Array.isArray(storyboards) && storyboards.length > 0) {
            bestBoard = storyboards.sort((a: any, b: any) =>
    ...
    try {
        ...
        if (err.stderr) console.error(`[Storyboard] stderr: ${err.stderr}`);
    } finally {
...
export const captureSnapshot = onCall({ memory: '1GiB', timeoutSeconds: 300, secrets: [TRANSCRIPT_PROXY_URL] }, async (request) => {
    const { data, auth } = request;
    ...
    const duration = data?.duration;   // total video seconds (float, optional)
